...
/* 
 * Script is used for characters_limit extenstion in Product Addons plugin
 * @author Piotr Szczygiel
 */

jQuery(document).ready(function(){
    
    var affected_types = ['custom', 'textarea'];

    // show or fide the input field when changing type
    jQuery('.jigoshop_addons select[name^=addon_type]').live('change', function() {
        var dates_min_max = jQuery(this).closest('.jigoshop_addon').find('.jpacl-addon');
        if (jQuery.inArray(jQuery(this).val(), affected_types) >= 0) {
            dates_min_max.show();
        }
        else {
            dates_min_max.hide();
    ...
    // display the type after loading the page
    jQuery('.jigoshop_addons select[name^=addon_type]').each(function() {
        if (jQuery.inArray(jQuery(this).val(), affected_types) >= 0) {
            jQuery(this).closest('.jigoshop_addon').find('.jpacl-addon').show();
        }
    });
